...
static void *extentDirNDMesh(int sel, Geom * g, va_list * args)
{
  ...
  if ((flag) || (numpts <= RANDOMTEST))
    for (i = 1; i < numpts; i++) {
      HPtNTransform(t, (((NDMesh *) g)->p)[i], hpt);
      HPtNDehomogenize(hpt, hpt);
      point = hpt->v + 1;
      temp = InnerProductN(point, direction, smdim);
      results[0] = (results[0] > temp) ? results[0] : temp;
    ...
    for (i = 0; i < RANDOMTEST; i++) {
      HPtNTransform(t, (((NDMesh *) g)->p)[test[i]], hpt);
      HPtNDehomogenize(hpt, hpt);
      point = hpt->v + 1;
      temp = InnerProductN(point, direction, smdim);
      results[0] = (results[0] > temp) ? results[0] : temp;
...
/* dimdir/direction are without the projective dimension, t may be NULL */
