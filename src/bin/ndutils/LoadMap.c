...
{	int i,j,k;
	...
	{	for(findend=ptr;*findend!='"';findend++);
		(map->coordsys)[j]=(char *)malloc
			((sizeof *((map->coordsys)[j]))
				*(((size_t)findend-(size_t)ptr)+1));
		memcpy((map->coordsys)[j],ptr,(sizeof *((map->coordsys)[j]))*
			((size_t)findend-(size_t)ptr));
